...
void read_BT() {
	if (Bluetooth.available() > 0) {
		...
		if (data == 'S') { // Segment
			...
			while (Bluetooth.available() == 0) {} // wait

			data = Bluetooth.read();

			while (data != 'R') {
				panel = (panel * 10) + number_char_to_int(data);
				while (Bluetooth.available() == 0) {} // wait
				data = Bluetooth.read();
			}
			while (data != 'G') {
				color = (color * 10) + number_char_to_int(data);
				while (Bluetooth.available() == 0) {} // wait
				data = Bluetooth.read();
			}
			...
			segment_color[3] = color;

			/*
			Serial.print('S');
			Serial.println(panel);
			Serial.print('R');
			Serial.println(segment_color[0]);
			Serial.print('G');
			Serial.println(segment_color[1]);
			Serial.print('B');
			Serial.println(segment_color[2]);
			Serial.print('W');
			Serial.println(segment_color[3]);
			Serial.println('#');
			*/
